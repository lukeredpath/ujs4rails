...
class ControllerWithControllerMethodsInjected < Test::Unit::TestCase
  ...
  def test_should_add_a_before_filter_that_creates_a_new_behaviour_script
    ...
    assert_instance_of UnobtrusiveJavascript::BehaviourScript, assigns(:js_behaviours)
  ...
  def test_should_add_an_after_filter_that_stores_the_behaviour_script_in_the_session
    assert ControllerStub.after_filters.include?(:store_js_behaviours)
    assert_equal session[:js_behaviours], assigns(:js_behaviours)
  end
